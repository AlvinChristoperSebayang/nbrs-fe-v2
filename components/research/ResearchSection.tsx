...
export function ResearchSection({ heading, subheading, sectors, practices, items }: ResearchSectionProps) {
  ...
  const filteredItems = visibleItems.filter((item) => {
    const matchesSector = selectedSectors.length === 0 || item.sectorSlugs.some((slug) => selectedSectors.includes(slug));
    const matchesPractice = selectedPractices.length === 0 || item.practiceSlugs.some((slug) => selectedPractices.includes(slug));
    return matchesSector && matchesPractice;
  });

  const toggleSector = (slug: string) => setSelectedSectors((current) => current.includes(slug) ? current.filter((value) => value !== slug) : [...current, slug]);
  const togglePractice = (slug: string) => setSelectedPractices((current) => current.includes(slug) ? current.filter((value) => value !== slug) : [...current, slug]);
