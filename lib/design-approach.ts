...
type Entry = {
  ...
  designApproachCommunitiesHeading: string | null; designApproachCommunitiesDescription: string | null;
  gallery: Array<{ image: Asset[] }>;
  thumbnailImage: Asset[]; quote: string | null; citation: string | null; jobRole: string | null;
  sectionHeading: string | null; text: string | null; image: Asset[];
...
export const DESIGN_APPROACH_FALLBACK: DesignApproachContent = {
  ...
  pillars: { title: "Three Pillars of Design", description: "Every NBRS project - whether a school, civic centre, hospital or secure space - follows a consistent methodology that adapts to context but never loses its foundation. We call it our Three Pillars:", items: [
    ...
    { title: "The Big Idea", description: "A unifying concept guiding decision making, which brings to life a balanced, joyful design.", image: "/images/design-approach/pillar3.jpg" },
  ] },
  communities: { heading: "Designing with Communities", description: "Through facilitated conversations and workshops, we uncover the social, cultural and emotional layers that inform meaningful, place-specific solutions.", topImages: ["/images/design-approach/communities1.jpg", "/images/design-approach/communities2.jpg"], galleryImages: ["/images/design-approach/communities3.jpg", "/images/design-approach/communities4.png", "/images/design-approach/communities5.jpg"] },
  quote: { image: "/images/design-approach/andrew-duffin.jpg", quote: "Architecture should never be imposed. It should emerge through dialogue.", author: "Andrew Duffin", role: "Director of Design" },
  project: { heading: "From Possibility to Place", description: "We follow through with discipline and detail to ensure every design achieves its intended impact.", image: "/images/design-approach/possibility.png", buttonText: "View Projects", buttonHref: "/projects" },
  cta: { image: "/images/contact-bg.png", title: "Get in touch", description: "Whether it's a place to gather, to heal, to learn or to live - we're ready to collaborate. Let's shape spaces that matter, together.", buttonText: "Contact Us", buttonHref: "/contact" },
};
...
query DesignApproachPage { entries(section: ["designApproach"], limit: 1) { ... on designApproach_Entry {
  ...
  designApproachCommunitiesHeading designApproachCommunitiesDescription
  gallery { ... on slide5_Entry { image { ${landscape} } } }
  thumbnailImage { mobile: ${crop(600, 750)} tablet: ${crop(900, 900)} desktop: ${crop(1200, 900)} }
  quote citation jobRole sectionHeading text image { ${landscape} }
  links { ... on links_Entry { linkText linkUrl } }
  ctaSection { ctaSectionBackgroundImage { ${cta} } ctaSectionHeading ctaSectionDescription ctaSectionButtonLabel ctaSectionButtonUrl }
} } }`;

function text(value: string | null): string | null { return value?.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim() || null; }
function path(value: string | null): string | null {
  if (!value) return null;
  try { const url = new URL(value); return /(^|\.)nbrs(-staging)?\.test$/.test(url.hostname) || /(^|\.)nbrs\.com\.au$/.test(url.hostname) ? `${url.pathname}${url.search}${url.hash}` : value; } catch { return value; }
}
